...
> Infraestructura Linux · Docker · MariaDB · WordPress · Replicación

---

## 📑 Índice

- [1. Máquina virtual](#1-máquina-virtual)
- [2. Conectividad y acceso remoto](#2-conectividad-y-acceso-remoto)
- [3. Docker Engine](#3-docker-engine)
- [4. Docker Compose](#4-docker-compose)
- [5. Directorio del proyecto](#5-directorio-del-proyecto)
- [6. Protección de información sensible](#6-protección-de-información-sensible)
- [7. Infraestructura Docker y bases de datos](#7-infraestructura-docker-y-bases-de-datos)
  - [7.1 Red Docker](#71-red-docker)
  - [7.2 Directorios para persistencia](#72-directorios-para-persistencia)
  - [7.3 Configuración de MariaDB principal](#73-configuración-de-mariadb-principal)
  - [7.4 Configuración de MariaDB réplica](#74-configuración-de-mariadb-réplica)
  - [7.5 Configuración en Docker Compose](#75-configuración-en-docker-compose)
  - [7.6 Despliegue de las bases de datos](#76-despliegue-de-las-bases-de-datos)
  - [7.7 Configuración del usuario de replicación](#77-configuración-del-usuario-de-replicación)
  - [7.8 Obtención de la posición del binlog](#78-obtención-de-la-posición-del-binlog)
  - [7.9 Configuración de la réplica](#79-configuración-de-la-réplica)
  - [7.10 Verificación de la replicación](#710-verificación-de-la-replicación)
  - [7.11 Prueba real de replicación](#711-prueba-real-de-replicación)
  - [7.12 Limpieza de la prueba](#712-limpieza-de-la-prueba)
- [8. Aplicación web — WordPress](#8-aplicación-web--wordpress)
- [9. Comprobación de persistencia de la base de datos](#9-comprobación-de-persistencia-de-la-base-de-datos)
- [10. Comprobación de puertos y servicios](#10-comprobación-de-puertos-y-servicios)
...
> ✅ **Resultado:** La prueba obtuvo un **0 % de pérdida de paquetes**.
...
> ✅ **Estado:** Docker está activo y funcionando correctamente.
...
La instalación se verificó mediante: Bash
...
El directorio pertenece al usuario zadmin y será utilizado para almacenar los archivos de configuración, documentación y posteriormente los datos persistentes de los servicios. 6. Protección de información sensible
...
Se creó una red privada denominada rotborn\_net para permitir la comunicación entre los contenedores sin necesidad de publicar sus puertos de base de datos directamente en el servidor: Bash
...
Se crearon directorios independientes para almacenar los datos de las dos instancias MariaDB: Bash

sudo mkdir -p /srv/rotborn/db sudo mkdir -p /srv/rotborn/db-replica
...
Antes del despliegue se verificó la configuración de Docker Compose mediante: Bash
...
Se iniciaron los servicios mediante Docker Compose: Bash

```bash
docker compose up -d mariadb mariadb-replica
```

Se comprobó posteriormente el estado de los contenedores: Bash
...
En la MariaDB principal se creó el usuario técnico utilizado por la réplica: SQL

CREATE USER 'replicator'@'%' IDENTIFIED BY '[CONTRASEÑA]'; GRANT REPLICATION SLAVE ON *.* TO 'replicator'@'%'; FLUSH PRIVILEGES;
...
En la MariaDB réplica se configuró la conexión con el servidor principal mediante: SQL

CHANGE MASTER TO MASTER\_HOST='mariadb', MASTER\_USER='replicator', MASTER\_PASSWORD='[CONTRASEÑA]', MASTER\_LOG\_FILE='mariadb-bin.000002', MASTER\_LOG\_POS=802;
...
Se comprobó el estado mediante: SQL
...
Para comprobar que la replicación funcionaba realmente se creó una tabla de prueba en la base de datos principal: SQL

CREATE TABLE prueba\_replicacion ( id INT PRIMARY KEY, mensaje VARCHAR(100) );

INSERT INTO prueba\_replicacion (id, mensaje) VALUES (1, 'Prueba de replicacion Rotborn');

Posteriormente se accedió a la réplica y se comprobó el contenido: SQL

USE rotborn; SELECT \* FROM prueba\_replicacion;
...
Una vez verificada la replicación, se eliminó la tabla utilizada para la prueba desde la base de datos principal: SQL

USE rotborn; DROP TABLE prueba\_replicacion;

Se comprobó en la MariaDB réplica que la eliminación también había sido replicada: SQL
...
La configuración se validó mediante: Bash
...
WordPress se inició mediante: Bash

docker compose up -d wordpress

Se comprobó el estado de los servicios mediante: Bash
...
El servicio web es accesible desde: Plaintext

http://IP\_DEL\_SERVIDOR:8080

(En este proyecto, la dirección utilizada fue [http://10.21.4.82:8080](http://10.21.4.82:8080/))
...
> ✅ **Resultado:** La instalación finalizó correctamente y WordPress se comunica exitosamente con la MariaDB principal.
...
Se comprobó el estado de los contenedores: Bash

```bash
docker compose ps
```

A continuación, se accedió a la MariaDB principal: Bash

```bash
docker exec -it rotborn-mariadb-1 mariadb -uroot -p
```

Se consultaron las tablas de la base de datos rotborn: SQL
...
Resultado (12 tablas de WordPress): wp\_commentmeta, wp\_comments, wp\_links, wp\_options, wp\_postmeta, wp\_posts, wp\_term\_relationships, wp\_term\_taxonomy, wp\_termmeta, wp\_terms, wp\_usermeta, wp\_users.

También se realizó una comprobación mediante una consulta sobre information\_schema: SQL

SELECT COUNT(\*) AS tablas FROM information\_schema.tables WHERE table\_schema = 'rotborn';
...
Se verificaron los puertos utilizados tanto por la máquina virtual como por los contenedores Docker mediante ```bash
docker compose ps
```:
...
También se comprobaron los puertos en escucha de la máquina virtual mediante: Bash

```bash
sudo ss -tulnp | grep -E ':22|:8080'
```

Resumen de servicios y puertos Servicio Puerto Expuesto Función SSH `22/TCP` Sí Administración remota de la VM WordPress `8080/TCP` Sí Aplicación web MariaDB `3306/TCP` No Base de datos principal interna MariaDB réplica `3306/TCP` No Réplica interna
...
La configuración documentada deja operativos los componentes principales del entorno:

- Máquina virtual Ubuntu Server 22.04.5 LTS
- Acceso remoto mediante SSH
- Docker Engine y Docker Compose
- MariaDB principal
- MariaDB réplica con replicación verificada
- WordPress conectado a la base de datos principal
- Persistencia mediante bind mounts
- Verificación de puertos y servicios

> **Nota de seguridad:** las contraseñas utilizadas durante la configuración no se incluyen en la documentación ni en el repositor
